...
public final class AurexFeatures {
    ...
    // Локальные подарки: отправка подарка становится локальной симуляцией, а
    // стоимость списывается с отдельного локального счётчика звёзд. Реальный
    // баланс Telegram Stars при этом не читается и не изменяется вообще никогда.
    // Настройка одна на клиент, но израсходованное и сами подарки хранятся
    // отдельно для каждого аккаунта (см. LocalGiftsStore).
    public static final BoolPref LOCAL_GIFTS = new BoolPref("paid_local_gifts", false);
