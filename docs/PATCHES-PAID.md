...
- изменяющие `TL_payments_*StarGift*` — улучшение, передача, перепродажа;
...
за две-три отправки кэш вырастал до сотен мегабайт. Теперь имена, начинающиеся
с `TL_payments_get` и `TL_stars_get`, пропускаются сразу: чтение звёзд не тратит.
...
### 3.8 Отображение в чате и порядок отправки — без новых врезок
...
`getDocument()` и хранится отдельно, а для показа собирается обычный
`TL_starGift` с этим документом в `sticker`. Обычный подарок и NFT идут по одной
и той же проверенной ветке отрисовки, с одинаковой штатной анимацией.
