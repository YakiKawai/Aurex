...
В работе замена этой врезки на вызов `AurexSpyChat.addHistoryItem(...)`: фасад сам
найдёт позицию пункта «Удалить» в готовом меню и вставит «Историю правок»
строго под ним. Причина: пункт `OPTION_DELETE` добавляется в пяти независимых
ветках `createMenu`, и единственный способ гарантировать один и тот же порядок
пунктов во всех типах чатов — вставлять по индексу в уже собранный список.
